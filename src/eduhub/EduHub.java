...
public class EduHub extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        Parent splashRoot;
        try {
            splashRoot = FXMLLoader.load(getClass().getResource("splashScreen.fxml"));
            Scene splashScene = new Scene(splashRoot, 600, 400); // Set the splash screen size
            splashStage.setScene(splashScene);

            // Calculate X and Y coordinates to center the splash screen
            double screenWidth = Screen.getPrimary().getBounds().getWidth();
            double screenHeight = Screen.getPrimary().getBounds().getHeight();
            double xPos = (screenWidth - 600) / 2; // Adjusted for the splash screen width
            double yPos = (screenHeight - 400) / 2; // Adjusted for the splash screen height
        ...
        Thread initThread = new Thread(() -> {
            try {
                Thread.sleep(5000); // Simulate 5 seconds of initialization time
    ...
    private void loadMainApp(Stage primaryStage) {
        try {
            ...
            Parent root = FXMLLoader.load(getClass().getResource("getStartedWIndow.fxml"));
